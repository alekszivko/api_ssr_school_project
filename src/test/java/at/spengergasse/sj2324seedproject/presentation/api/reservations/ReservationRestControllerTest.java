...
public class ReservationRestControllerTest {
  ...
  void ensureFetchAllReturnsNoContentForMissingData() throws Exception {
    when(reservationService.fetchReservations(Optional.empty())).thenReturn(
  ...
  void ensureFetchAllReturnsContentForExistingData() throws Exception {
    ...
    when(reservationService.fetchReservations(Optional.of(reservation.isCompleted())))
        .thenReturn(List.of(reservation));

    var request = get(
        ReservationRestController.BASE_URL).param("completed",
  ...
  @Test
  void ensureCreateReservationProblemDetailInInternalServerErrorResponseDueToPersistenceException()
      throws Exception {
    //given

    var description = "description";
    when(reservationService.createReservation(eq(description), any())).thenThrow(
  ...
  void ensureGetReservationByIDReturnsReservation() throws Exception {
    ...
    when(reservationService.getReservationByReservationID(reservation.getReservationId()))
        .thenReturn(Optional.of(reservation));

    var request = get(ReservationRestController.BASE_URL + "/{id}",
        reservation.getReservationId()).accept(MediaType.APPLICATION_JSON);
